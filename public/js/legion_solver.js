...
class LegionSolver {
      ...
      if (
        ...
        if (!this.firstAlgorithm) {
          ...
          while (
            ...
            this.board[this.emptySpots[position].y][this.emptySpots[position].x] != 0
          ) {
            position++;
          }
        } else {
        ...
        if (this.directionFree == 0) {
          ...
          this.takeBackPiece(
            point,
            this.pieces[this.pieceNumber].transformations[this.transformationNumber]
          );
        } else {
  ...
  changeIndex(restricted) {
    if (restricted) {
      if (
        this.restrictedTransformationNumber <
        this.pieces[this.restrictedPieceNumber].restrictedTransformations.length - 1
      ) {
        this.restrictedTransformationNumber++;
      } else {
        this.restrictedPieceNumber++;
